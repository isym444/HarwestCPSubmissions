...
ll minCoins(const vector<ll>& coins, ll V) {
    vector<int> dp(V + 1, 0);
    dp[0] = 0; // Base case

    for (ll i = 1; i <= V; ++i) {
        dp[i] = i; // Max coins needed would be all 1's, hence initializing to i
        for (ll coin : coins) {
            if (coin <= i) {
...
int main() {
    ...
    foj(0,t){
        ...
        ll ans=0;
        if(target>100){
            ll temp=floordiv((target-30),15);
            ans+=temp;
            target-=15*temp;
        }
        ans+=minCoins(coins, target);
        cout << ans << endl;
    }
